...
void memWriterReader::nbMemWR(){
	...
	for (int i = 0; i < 25; i = i + 8){
		...
		if (cmd == tlm::TLM_WRITE_COMMAND) {
			...
			data[3] = (i+4); data[4] = (i+5); data[5] = (i + 6);
			data[6] = (i + 7); data[7] = (i + 8);
		}
		nBlockWriteRead->set_command( cmd );
		nBlockWriteRead->set_address( i );
		nBlockWriteRead->set_data_ptr( (unsigned char*) data );
		nBlockWriteRead->set_data_length( 8 );
		nBlockWriteRead->set_streaming_width( 8 );
